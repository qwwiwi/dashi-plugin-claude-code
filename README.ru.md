...
> **Статус:** в production на мультиагентном флоте. Текущий релиз — **[v1.1.0](CHANGELOG.md)** (2026-07-04): rich messages, Guest Mode, context HUD + закреплённая статус-карточка — см. раздел [15](#15-что-нового-в-v110--rich-messages-guest-mode-context-hud--статус-пин). Версионирование и процесс релизов: [CHANGELOG.md](CHANGELOG.md) · [docs/RELEASING.md](docs/RELEASING.md). CI: `bun test` + `bun run typecheck` обязаны проходить чисто перед merge.
...
Per-chat persona-оверлей накладывает SessionStart-хук `chats/hooks/session-start.sh`: он читает persona-файл чата (резолвится относительно `multichat.workspace_dir`) внутри tmux-сессии и инжектит его поверх единой identity через `additionalContext` -- никаких отдельных `CLAUDE.md` per chat не нужно. Логи: `{state_dir}/chats/<chat_id>/{inbox,outbox,processing,dead-letter}/*.json`.
...
Ключевое по релизам, новое сверху. Полная история: [CHANGELOG.md](CHANGELOG.md); как выпускаются релизы: [docs/RELEASING.md](docs/RELEASING.md).

### v1.2.0 (2026-07-10) -- HUD по модели сессии, зеркало задач в пине, полировка вывода

Вышло в середине июля 2026 (PR #100–#108).

#### Context HUD учитывает модель сессии (PR #106, #107)
...
- **Оверрайд остался и всегда побеждает:** окно можно задать явно -- конфиг `context_window_tokens` или env **`JARVIS_CONTEXT_WINDOW`**; явное операторское значение бьёт таблицу моделей, так что неверная догадка таблицы правится на лету.
...
Единая закреплённая статус-карточка умеет **раскрываться** в полный текущий список задач, а снимок задач **сбрасывается при старте сессии**, так что свежая сессия больше не наследует задачи предыдущей. Зеркало реальности из tmux-пейна читает настоящий список задач харнесса прямо из пейна и подаёт пину именно эту pane-verified картину -- то, что видно в пине, совпадает с реальными задачами сессии, а не только с событиями из хуков.

#### Формат Telegram-вывода (PR #105)

Сохранение переносов строк в rich-пути, heading-affinity чанкер (заголовок остаётся вместе с блоком, который он вводит, когда длинное сообщение режется на части) и задокументированный контракт tone-of-voice ([plugin/docs/TOV.md](plugin/docs/TOV.md)).

#### Фиксы и гигиена (PR #101, #102, #103, #108)

- **Классификатор пейна под v2.1.201:** `classifyPane` распознаёт busy-спиннер Claude Code v2.1.201, так что работающая сессия не читается как idle.
- **Zoom-ссылки:** `pwd=` в join-URL на `zoom.us` -- это публичный код входа, а не секрет, и он не редактируется; экземпция ужесточена allowlist-ом параметров (`pwd`, `uname`, `omn`) и стрипом сырых NUL-байт.
- **Точный permission-гейт:** детектор git-exec-surface больше не поднимает ложные карточки подтверждения на безобидный git, сохраняя защиту от RCE.
- **Рефакторинг репо:** `webhook/server.ts` разбит на route-модули, неиспользуемый `persona-manager.ts` удалён (persona-оверлей накладывает SessionStart-хук `chats/hooks/session-start.sh`), tracked-мусор вычищен, `.gitignore` ужесточён.

### v1.1.0 (2026-07-04) -- rich messages, Guest Mode, context HUD + статус-пин

Вышло в начале июля 2026 (PR #86, #94–#97).

#### Rich messages (Bot API 10.1)

DM-ответы автоматически апгрейдятся до `sendRichMessage`: Telegram рендерит **сырой markdown на своей стороне** — нативные заголовки, таблицы, task-lists, математика, свёртки `<details>`, сноски — а лимит тела **32 768 байт** (против 4 096 у обычного сообщения), так что длинный структурный ответ уходит ОДНИМ сообщением, без нарезки на чанки и потерь при HTML-конверсии.

- **Zero-config.** Включать нечего: подходящий ответ (личка, без вложений, помещается в 32 KB) сначала пробует rich-путь.
- **Прозрачный фолбэк — сообщение не теряется никогда.** Если Telegram (или локальная сборка grammY) не знает метода, session-латч выключает rich, и всё идёт классическим HTML-путём. Отказ парсера markdown (400) — разовый откат без латча; превышение размера — откат в HTML-чанкинг.
- **Безопасность без изменений.** Rich-путь проходит те же обёртки редакции секретов и rate-limit, что и любая другая отправка, — это новый метод того же слоёного Telegram API.
- **Управление:** киллсвитч — env `TELEGRAM_RICH_MESSAGES=0` или конфиг `richMessages.enabled=false`; per-chat опт-аут — конфиг `richMessages.perChatOptOut` или env `TELEGRAM_RICH_MESSAGES_PER_CHAT_OPT_OUT` (CSV chat id).
- **Область действия:** пока только личка. Группы/multichat и ответы Guest Mode остаются на HTML-пути.

#### Guest Mode (one-shot @-упоминания)

@-упомяни бота в **любом чате, где его НЕТ**, — и агент получает one-shot запрос: входящее сообщение несёт `guest="1"` и `guest_query_id`, а ответ доставляется в тот чужой чат через `answerGuestQuery` — и виден всем участникам.

- **Fail-closed:** вызвать бота могут только user id из allowlist; остальные отбрасываются до любого хендлера.
- **One-shot контракт:** ровно один ответ на запрос, ≤4096 символов, без вложений и threading; запрос истекает примерно через 15 минут.
- **Для включения нужны оба переключателя:** Guest Mode toggle в BotFather И `guest_mode.enabled=true` в конфиге (отсутствие блока = выключено).

#### Context HUD + одна закреплённая статус-карточка

В личке владельца ровно **одно закреплённое служебное сообщение**: процент контекстного окна, режим сессии и текущие задачи; карточка пере-якорится над terminal mirror на каждое входящее. Служебные пузыри автоудаляются; меню команд видно только владельцу; `/compact`, `/new`, `/status` — state-aware (управляют живым TUI). Диалоги `AskUserQuestion` транслируются как **инлайн-кнопки** Telegram — на вопрос агента с вариантами отвечаешь одним тапом. (В v1.2.0 знаменатель контекстного процента стал учитывать модель сессии — см. выше.)

Конфиг: `hud.enabled` (опциональный блок; отсутствие блока = включено). Чаты владельца берутся из owner-allowlist — HUD и его кнопки никогда не появляются в группах.
