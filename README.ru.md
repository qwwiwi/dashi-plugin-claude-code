...
Per-chat persona-оверлей накладывает SessionStart-хук `chats/hooks/session-start.sh`: он читает persona-файл чата (определяется относительно `multichat.workspace_dir`) внутри tmux-сессии и добавляет его поверх единой identity через `additionalContext` — никаких отдельных `CLAUDE.md` per chat не нужно. Логи: `{state_dir}/chats/<chat_id>/{inbox,outbox,processing,dead-letter}/*.json`.
...
- **Zoom-ссылки:** параметр `pwd=` в распознанном join-URL на `zoom.us` намеренно сохраняется, чтобы расшаренные ссылки на встречу оставались рабочими; исключение ограничено allowlist-ом параметров (`pwd`, `uname`, `omn`), а удаление сырых NUL-байт на входе смягчает подмену плейсхолдера.
...
- **Рефакторинг репо:** `webhook/server.ts` разбит на route-модули, неиспользуемый `persona-manager.ts` удалён (persona-оверлей накладывает SessionStart-хук `chats/hooks/session-start.sh`), отслеживаемый мусор вычищен из индекса git, `.gitignore` ужесточён.
...
- **Входящее медиа (v1.3.0):** агент получает медиа самого упоминания *и* процитированного сообщения на паритете с личкой — фото скачивается, голосовые расшифровываются, файлы/видео/GIF отдаются как `file_id` `<media>`-дескрипторы. Медиа цитаты — только метаданные (автор цитаты не в allowlist). Байты любого guest-вложения тянутся тулом `download_attachment` с тем же `guest_query_id`.
